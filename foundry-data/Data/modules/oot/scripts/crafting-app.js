...
export class CraftingApplication extends Application {
  ...
  async _performCraftingCheck(skillConfig, checkName, existingBoons = 0) {
    const dc = CRAFTING_CONFIG.rarityDC[this.selectedRarity];

    const rollResult = await this._performSkillRoll(skillConfig, checkName);
    ...
    if (!outcome.destroyed) {
      if (outcome.flaws > 0) {
        flaws = rollFlaws(outcome.flaws, this.selectedItemType);
      }
      if (outcome.boons > 0) {
        const maxBoons = CRAFTING_CONFIG.maxEnchantingBoons[this.selectedRarity];
        const remainingBoons = Math.max(0, maxBoons - existingBoons);
        boons = rollBoons(Math.min(outcome.boons, remainingBoons), this.selectedItemType, remainingBoons);
      }
    }
    ...
    await this._postCheckResultToChat(checkName, skillConfig, result);

    return result;
  }

  async _postCheckResultToChat(checkName, skillConfig, result) {
    const actor = this.craftingActor;
    const itemName = this.selectedItem?.name || "Unknown Item";
    const rarity = CRAFTING_CONFIG.rarityLabels[this.selectedRarity];
    ...
    if (result.destroyed) {
      outcomeClass = "destroyed";
    } else if (result.boons.length > 0) {
      outcomeClass = "success";
    } else if (result.flaws.length > 0) {
    ...
    if (result.destroyed) {
      ...
      if (result.flaws.length > 0) {
        content += `<div class="quirks flaws"><strong>Flaws:</strong><ul>`;
        for (const flaw of result.flaws) {
          content += `<li><strong>${flaw.name}</strong> (d20: ${flaw.roll}): ${flaw.description}</li>`;
        }
        content += `</ul></div>`;
      }

      if (result.boons.length > 0) {
        content += `<div class="quirks boons"><strong>Boons:</strong><ul>`;
        for (const boon of result.boons) {
          content += `<li><strong>${boon.name}</strong> (d20: ${boon.roll}): ${boon.description}</li>`;
        }
        content += `</ul></div>`;
      }
  ...
  async _performSkillRoll(skillConfig, flavor) {
    const actor = this.craftingActor;
    ...
    if (skillConfig.type === "skill") {
      ...
      rollResult = this._extractRollResult(rollData);
    } else {
      const toolItem = actor.items.find(i =>
        i.type === "tool" &&
        i.name.toLowerCase().includes(skillConfig.name.toLowerCase().replace("'s tools", "").replace("'s supplies", "").trim())
      );
      ...
      if (toolItem) {
        ...
        rollResult = await new Roll(`1d20 + ${profBonus}`).evaluate();
        await rollResult.toMessage({
          speaker: ChatMessage.getSpeaker({ actor }),
          flavor: `${flavor} (${skillConfig.name})`
        });
      }
    }
